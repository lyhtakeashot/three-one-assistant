...
3. **页面底色加固**：`html` 与 `#root` 追加 `background:var(--bg)`，并增加 `html,body{background:#F8FAFC}` 显式兜底，避免只绘制 `html` 背景的预览 / webview 透出宿主底色。**未改** 任何主题 token 色值，`favicon` 按用户要求未改。
...
9. **改完 `index.html` 必须升 `sw.js` 的 `CACHE_NAME`**（现 `3in1-v14`），否则用户拿不到新版本外壳。
