...
> **当前状态一句话**：数据与功能均已最新（**38 所三位一体口径 + 类别维度 + 下载中文化**），**但本轮全部改动尚未 commit / push**。
> 远端 `main` 停在 `0d4f1e2`；工作区有 **18 个已跟踪文件**待提交（详见 §9）。
...
5. **测试基线 `4582 passed / 0 failed`**，任何改动后必须保持 0 failed（见 §4.3 与附录 B）。
...
├── sw.js             # PWA Service Worker，CACHE_NAME = '3in1-v12'
├── manifest.json / icon.*
...
**仍未跟踪（未 `git add`）**：`.三位一体数据采集表.ref/`、`溯源素材/`、`三位一体数据采集表.xlsx`、`三位一体数据采集表-2025.xlsx`、`三位一体数据采集需求表.md`、`三位一体数据溯源采集需求表.md`、`.codebuddy/plans/`、`.workbuddy/`、`live-app/data/`。

> ⚠️ **`.gitignore` 缺口**：只忽略了 `three-one-assistant/data/`，**没有忽略 `live-app/data/`**（本地运行时 JSON，含用户提交内容）。`git add -A` 会误提交，建议补一条 `live-app/data/`。
...
- [ ] **提交并推送本轮 18 个文件**（见 §9）；push 会触发 Cloudflare 部署。
- [ ] **补 `.gitignore`**：加 `live-app/data/`（避免运行时数据与用户提交内容入库）。
...
## 附录 B：测试套件（7 组，共 4582 项）

| 组 | 项数 | 覆盖 |
|---|---|---|
| `data.test.cjs` | 3985 | 院校字段完整性、URL、满意度范围、学考折算递减、权重和=1、**范围/类别/年份视图**、备查校不伪造 |
...
9. **改完 `index.html` 必须升 `sw.js` 的 `CACHE_NAME`**（现 `3in1-v12`），否则用户拿不到新版本外壳。
