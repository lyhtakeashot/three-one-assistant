...
- `sw.js` `CACHE_NAME` → **`3in1-v17`**；回归 **4585 passed / 0 failed**；线上复核（2026-09-17）：`sw.js` 已为 `3in1-v17`、`/api/counter` 正常返回、首页已含 `3in1_counted` 会话标记代码。commit `5da0447`。

---

## 9. git 状态与提交指引（2026-09-17 实测）

```
## main...origin/main          ← 与远端同步（5da0447，无领先/落后）
5da0447 fix(counter): count once per tab session via sessionStorage guard against sw auto-reload; bump sw cache 3in1-v17
3c5b33f feat(counter): switch to visit-count semantics, remove uid dedupe; align local server; bump sw cache 3in1-v16
c681af9 refactor(sidebar): remove redundant brand-logo icon; keep brand-text; bump sw cache 3in1-v15
1e4833c fix(ui): page background solid white in light to match topbar; dark unchanged
```

工作区仅有两个未跟踪的 `.三位一体数据采集表.ref/` 临时文件（`jxnh_text.txt` / `jxnh_tmp.pdf`），与代码无关，可忽略或手动清理。
...
11. **计数接口防重复要靠前端会话标记（2026-09-17 两轮迭代后定稿）**：`/api/counter` 语义为「每次请求无条件 `count+1`」（线上 handler 与本地 `server.cjs` 一致），防重复完全由前端 `getUserCounter` 的 **sessionStorage 标记（键 `3in1_counted`）** 承担：每个标签页会话只计 1 次，刷新 / SW 更新自动重载不重复计，新开标签页才再计；已计数会话读 localStorage 缓存（`3in1_counter`）展示。历史教训：① 曾因永远带 `userId` 导致「永不自增」（服务端只在无 `userId` 时 +1）；② 改成「每次页面加载 +1」后，又被 SW 发版自动强刷（`controllerchange`→`reload`）放大为「每次打开 +2」。**该函数在切片区内，新增代码严禁 `window`/`document`；sessionStorage 访问必须 try/catch（沙箱只注入 `localStorage`）**。
